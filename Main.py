...
class ByteManager:
    ...
    def byte_to_nibbles(cls, byte):
        lower_nibble = byte & 0xF     #0xF == 1111
...
class KeyGenerator:
    Prime_DIGITS = [97829,45737,24019,18127,60611,49103,52253,51563,98491,86341,31543,66221]
    KEY_LENGTH = len(Prime_DIGITS)
    ...
    def __init__(self, user_remark):
        ...
        for x in range(KeyGenerator.KEY_LENGTH):
            self.numerickey[x] = (self.numerickey[x] + KeyGenerator.Prime_DIGITS[x]) % 256
...
def main():
    sfname = 'd:/c.mp4'
    efname = 'd:/e_c.mp4'
    tfname = 'd:/school_c.mp4'

    user_key = 'My name is Kuldeep , I am author of this program'
